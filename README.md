...
Run Agent 是一个用于学习 AI Agent 基本工作方式的轻量项目。程序把 Anthropic 模型与一组本地工具连接起来：模型可以读取、编辑和管理工作区文件，也可以执行 Shell 命令、维护 TODO 任务清单，并按需加载工作区中的 skills；程序会控制超长工具输出和消息历史规模，把每次工具执行结果返回给模型，直到模型不再请求工具。
...
│   ├── runtime.py       # Agent 调用循环、上下文压缩与 hooks 触发点
...
Docker 镜像只安装 `requirements.txt` 中的运行依赖并复制程序代码；测试源码、pytest 配置、开发依赖、测试缓存、转录和工具输出目录会被 `.dockerignore` 排除，以减小生产镜像的构建上下文。skills 从运行工作区 `/workspace/resources/skills` 扫描，不会预先打包进镜像。
...
此时 `read_file` 等读取操作以及 skills 加载可以正常使用，但 `write_file`、`edit_file`、`delete_file`、`copy_file` 和 `move_file` 无法修改挂载的文件。上下文摘要仍能工作，但转录和大工具结果无法写入挂载目录：程序会打印转录未保存提示，并保留无法落盘的原始工具输出。如果确实需要完整的文件管理及压缩产物持久化能力，可将挂载参数末尾的 `:ro` 改为 `:rw`；执行前请先确认目录中没有重要或敏感文件。
...
3. 请求模型前依次裁剪消息、微压缩旧工具结果并控制最新工具结果预算；仍超过阈值时保存转录并生成摘要。
4. 程序把系统提示词、整理后的消息和 `tools_config.py` 中的全部工具 JSON Schema 发送给模型。若服务端报告上下文溢出，则响应式压缩并重试一次。
...
- `.transcripts` 和 `.task_outputs` 可能包含模型上下文或完整工具输出，虽然默认不会提交到 Git，也应按照工作区原始数据的敏感级别管理和清理。
