...
- 每次模型请求前扫描 `resources/skills/*/SKILL.md`，把技能名称和简介加入系统提示词，并通过 `load_skill` 按需加载完整说明
...
- 模型输出被截断时先把上限从 8,000 升到 64,000，再通过最多 3 条续写提示恢复不完整回答
- 429 限流和 529 服务过载采用指数退避与随机抖动进行最多 10 次请求，并优先遵循服务端 `Retry-After`
- 连续 3 次收到 529 时可通过 `FALLBACK_MODEL_ID` 自动切换备用模型
- 支持 `.env` 环境变量、自定义 Anthropic API 地址和可选备用模型
...
`run_agent/prompt.py` 不再维护一整块硬编码提示词，而是通过 `PROMPT_SECTIONS` 分开保存不同职责的模板，再由运行时状态决定实际内容：

| Section | 加载策略 | 运行时来源 |
| --- | --- | --- |
| `identity` | 始终加载 | Agent 身份、操作说明和回答语言约束 |
| `tools` | 始终加载 | `TOOL_HANDLERS` 当前注册的工具名称 |
| `workspace` | 始终加载 | `WORKDIR` 对应的工作区边界 |
| `skills` | 始终加载 | 扫描 `resources/skills/*/SKILL.md` 得到的简短目录 |
| `memory` | 索引非空时加载 | `resources/memory/MEMORY.md` 的简短目录 |

`update_context()` 每次模型请求前重新扫描这些真实状态，不根据用户消息中的关键词猜测能力是否存在。`assemble_system_prompt()` 按固定顺序拼接 section；没有记忆索引时不会加入空的 memory 段。完整的相关记忆仍通过 `<relevant_memories>` 附加到用户消息的请求副本，system prompt 只携带简短索引和信任边界。

`get_system_prompt()` 使用 `json.dumps(..., sort_keys=True)` 生成确定性的上下文键。工具轮次之间状态没有变化时直接复用上一次字符串；工具新建或修改 skill、memory 索引等状态后，下一次模型请求会重新组装。该缓存只减少本地字符串拼接，不等同于 Anthropic API 的 prompt caching。`build_system()` 作为兼容入口仍可直接构建当前完整提示词。
...
`run_agent/recovery.py` 使用 `RecoveryState` 保存当前模型、输出上限和各恢复路径的使用次数。`runtime.py` 仍先发起普通模型请求，只有识别到明确故障后才执行对应的最小恢复动作：

| 故障 | 识别方式 | 恢复动作 | 上限 |
| --- | --- | --- | --- |
| 输出截断 | `stop_reason == "max_tokens"` | 首次不保存截断内容，把 `max_tokens` 从 8,000 提高到 64,000 并重试相同请求；再次截断时保存当前内容并追加直接续写提示 | 上限升级 1 次，续写提示 3 次 |
| 上下文超限 | HTTP 413 或常见 `prompt_too_long` / context length 错误标记 | 保存转录、生成响应式摘要并保留最近消息后重试 | 默认 1 次 |
| 限流 | HTTP 429 或 SDK `RateLimitError` | 按 `min(500 × 2^attempt, 32000)` 毫秒退避，再增加最多 25% 随机抖动 | 单次调用最多 10 次请求 |
| 服务过载 | HTTP 529 或 `overloaded_error` | 使用同样的退避策略；连续 3 次过载且配置了备用模型时切换模型 | 单次调用最多 10 次请求 |

如果响应头包含可解析的 `Retry-After` 秒数，429/529 会优先采用该值而不是本地退避公式。成功请求会清空当前瞬态故障计数；达到次数上限，或遇到鉴权失败等非恢复类异常时，原异常会继续抛出，不会被包装成模糊错误。输出达到续写限制时，历史会保留最后一段已生成内容并结束本次循环。
...
│   ├── prompt.py        # 系统提示词分段组装、运行时上下文刷新及结果缓存
...
| `MODEL_ID` | 是 | 调用的模型 ID |
| `FALLBACK_MODEL_ID` | 否 | 连续三次收到 529 服务过载时切换到的备用模型 ID |
...
1. 每次模型请求前读取工具注册表和工作区、扫描 skills、读取 `MEMORY.md`，将真实状态交给 system prompt 组装器；状态未变化时复用缓存结果。
...
4. 把相关记忆附加到整理后消息的请求副本，再将系统提示词、请求消息和 `tools_config.py` 中的全部工具 JSON Schema 发送给模型。429/529 会在请求包装器内退避重试；连续 529 可切换备用模型。
...
接口返回 429 或 529 时，运行时会将其视为可恢复的瞬态故障，并显示带 `[recover]` 的等待与重试日志。服务端提供 `Retry-After` 时优先按该秒数等待，否则使用 0.5 秒起步的指数退避和随机抖动；单次调用最多发起 10 次请求。只有连续三次收到 529 且 `.env` 配置了 `FALLBACK_MODEL_ID` 时才会切换备用模型，429、成功响应或其他异常不会触发切换。
