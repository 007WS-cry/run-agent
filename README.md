...
Run Agent 是一个用于学习 AI Agent 基本工作方式的轻量项目。程序把 Anthropic 模型与一组本地工具连接起来：模型可以读取、编辑和管理工作区文件，也可以执行 Shell 命令和维护 TODO 任务清单；程序会把每次工具执行结果返回给模型，直到模型不再请求工具。
...
- 支持 Bash、读取、编辑、写入、删除、复制、移动和 TODO 管理共 8 个工具
...
测试通过临时目录隔离文件操作，并模拟 Anthropic 客户端，因此不会访问真实模型 API，也不会改动项目中的业务文件。hooks 测试会模拟用户审批输入，验证禁止命令不会询问、破坏性命令默认拒绝，以及运行时不会执行被拦截的工具；TODO 测试会验证工具 Schema、任务状态替换、异常输入保护、三轮提醒和成功更新后的计数器重置。`pytest.ini` 会把临时测试文件放在项目内的 `.pytest_tmp` 目录，该目录已被 Git 和 Docker 忽略。
...
1. 用户提交问题后触发 `UserPromptSubmit`，随后问题被加入消息历史。
2. 程序把消息和全部工具的 JSON Schema 发送给模型。
3. 如果模型请求调用工具，先触发 `PreToolUse` 完成权限检查和调用日志记录。
4. 权限检查通过后，程序根据工具名称从 `TOOL_HANDLERS` 中找到对应函数并执行；被拒绝的调用会跳过执行。
5. 模型进入工具调用轮次时增加 TODO 提醒计数；成功执行 `todo_write` 时清零，累计三轮未更新时在下一次请求前插入提醒。
6. 工具执行完成后触发 `PostToolUse`，再把返回值作为 `tool_result` 加入消息历史并返回给模型。
7. 重复上述过程，直到模型给出最终回答；此时触发 `Stop` 输出会话工具调用统计。
