...
测试通过临时目录隔离文件操作，并模拟 Anthropic 客户端，因此不会访问真实模型 API，也不会改动项目中的业务文件。hooks 测试还会模拟用户审批输入，验证禁止命令不会询问、破坏性命令默认拒绝，以及运行时不会执行被拦截的工具。`pytest.ini` 会把临时测试文件放在项目内的 `.pytest_tmp` 目录，该目录已被 Git 和 Docker 忽略。
...
5. 工具执行完成后触发 `PostToolUse`，再把返回值作为 `tool_result` 加入消息历史并返回给模型。
6. 重复上述过程，直到模型给出最终回答；此时触发 `Stop` 输出会话工具调用统计。
