...
from run_agent import runtime, todos
...
def reset_todo_state(monkeypatch):
    monkeypatch.setattr(todos, "CURRENT_TODOS", [])
...
def test_todo_tool_schema_and_handler_are_registered():
    todo_tool = next(
        tool for tool in tools_config.TOOLS if tool["name"] == "todo_write"
    )
    schema = todo_tool["input_schema"]
    item_schema = schema["properties"]["todos"]["items"]

    assert tools_config.TOOL_HANDLERS["todo_write"] is todos.run_todo_write
...
def test_todo_write_replaces_state_and_prints_tasks(capsys):
    ...
    result = todos.run_todo_write(task_list)

    assert result == "Updated 3 tasks"
    assert todos.CURRENT_TODOS == task_list
    assert todos.CURRENT_TODOS is not task_list
    output = capsys.readouterr().out
    assert "[ ] 分析需求" in output
    assert "[\033[36m▸\033[0m] 编写实现" in output
    assert "[\033[32m✓\033[0m] 运行测试" in output
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [{"content": "任务", "status": "pending", "extra": True}],
            "only content and status",
        ),
    ],
)
# 验证异常输入会返回统一错误文本，并保留此前已经成功写入的任务清单。
def test_todo_write_rejects_invalid_input_without_changing_state(todo_input, message):
    previous = [{"content": "保留任务", "status": "pending"}]
    todos.CURRENT_TODOS = previous

    result = todos.run_todo_write(todo_input)

    assert result.startswith("Error:")
    assert message in result
    assert todos.CURRENT_TODOS is previous


@pytest.mark.parametrize(
    ("serialized_todos", "expected_content"),
    [
        ('[{"content": "JSON 任务", "status": "pending"}]', "JSON 任务"),
        ("[{'content': '字面量任务', 'status': 'completed'}]", "字面量任务"),
    ],
)
# 验证重复序列化的 JSON 数组和 Python 列表字面量都能被安全解析并更新为独立任务清单。
def test_todo_write_accepts_serialized_lists(serialized_todos, expected_content):
    result = todos.run_todo_write(serialized_todos)

    assert result == "Updated 1 tasks"
    assert len(todos.CURRENT_TODOS) == 1
    assert todos.CURRENT_TODOS[0]["content"] == expected_content
    assert todos.CURRENT_TODOS[0]["status"] in {"pending", "completed"}
...
def test_successful_todo_write_resets_reminder_counter(
    ...
    monkeypatch.setattr(
        runtime,
        "TOOL_HANDLERS",
        {"todo_write": todos.run_todo_write},
    )
    runtime.rounds_since_todo = 2
    tool_block = content_block_factory(
        "tool_use",
